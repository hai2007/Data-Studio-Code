 ...
 *
 * Date:Fri Jun 10 2022 10:00:39 GMT+0800 (中国标准时间)
 */
(function () {
  'use strict';
  ...
  function ruler (maxValue, minValue, num) {
    ...
    if (distance <= 0) return [minValue, maxValue]; // 最小值，也就是起点
  ...
  function analyseWord (express) {
    ...
    while (true) {
      if (reader.index >= express.length) break; // 单行注释
  ...
  function painter (platform, canvas, width, height) {
    ...
    if (platform != 'default') {
      ...
      canvas.style.height = height + "px"; // 设置画布大小（画布大小设置为显示的两倍，使得显示的时候更加清晰）
  ...
  function initMixin(Clunch) {
    ...
    Clunch.prototype.$$init = function (options) {
      ...
      this._debug = 'debug' in options ? options.debug : false; // 记录平台
    ...
    Clunch.prototype.$$updateWithData = function (noAnimation) {
      ...
      (function doit(renderAOP, pScope, isSubAttrs, pid, ignoreFor) {
        ...
        for (var i = 0; i < renderAOP.length; i++) {
          ...
          try {
            // c-if
            if ('c-if' in renderAOP[i] && !evalExpress(that, renderAOP[i]['c-if'], renderAOP[i].scope)) {// 如果c-if是false，就不用当前的就可以略过了
            } else {
              // 计算子组件
  ...
  Clunch.prototype.$mount = function (el) {
    ...
    if (this._platform == 'default') {
      if (this._debug) this.__el.setAttribute('__clunch__devtool__', new Date().valueOf() + "" + (Math.random() * 100000).toFixed(0)); // 初始化添加画布
  ...
  var calcWidth = function calcWidth(fontSize, content) {
    return content.length * fontSize;
  }; // 悬浮提示


  var hover = ['number', "string", 'color', function ($number, $string, $color) {
    return {
      attrs: {
        title: $string(''),
        x: $number(-1)(true),
      ...
      link: function link(painter, attr) {
        ...
        var maxWidth = 0;
        if (attr.title != '') maxWidth = calcWidth(16, attr.title);

        for (var i = 0; i < attr._subAttr.length; i++) {
          var tempWidth = calcWidth(12, attr._subAttr[i].attr.content) + 14;
          if (tempWidth > maxWidth) maxWidth = tempWidth;
        } // 获取最终绘制的大小


        var width = attr.width == -1 ? maxWidth + 35 : attr.width;
        var height = attr.title == '' ? 0 : 25;
        height += 20 + attr._subAttr.length * 14; // 校对位置

        var x;

        if (attr.x > this._width * 0.5) {
          x = attr.x - (width + 5);
        } else {
          x = attr.x + 5;
        }
        ...
        painter.config('fillStyle', attr.background);
        ...
        if (attr.title != "") {
          painter.config({
            'fillStyle': attr.color,
            'font-size': attr['title-size']
          }).fillText(attr.title, 10 + x, sizeY + 10 + y);
          sizeY += 25;
        } // 开始绘制小条目


        painter.config({
          'font-size': attr['item-size']
        });

        for (var _i = 0; _i < attr._subAttr.length; _i++) {
          painter.config({
            'fillStyle': attr.color
          }).fillText(attr._subAttr[_i].attr.content, 30 + x, sizeY + 7 + y).config({
            'fillStyle': attr._subAttr[_i].attr.color
          }).fillCircle(x + 17, y + sizeY + 7, attr._subAttr[_i].attr.radius);
          sizeY += 14;
        }
      }
    };
